...
function applyFilter() {
	...
	if (parseInt(document.getElementById("ftPCI1").value)!=lowPCI){
		alert("Your input for PCI is out of range!");
		document.getElementById("ftPCI1").value=lowPCI;
	}
	if (parseInt(document.getElementById("ftPCI2").value)!=upPCI){
		alert("Your input for PCI is out of range!");
		document.getElementById("ftPCI2").value=upPCI;
	}	
    ...
    if (filterCPList.length > 0) {
        ...
        if (isNaN(document.getElementById("frYR").value) == false) {
            if (parseInt(document.getElementById("frYR").value) >= 2005 && parseInt(document.getElementById("frYR").value) <= 2021) {
                lowYear=parseInt(document.getElementById("frYR").value)
            }
        }
        if (isNaN(document.getElementById("toYR").value) == false) {
            if (parseInt(document.getElementById("toYR").value) >= 2005 && parseInt(document.getElementById("toYR").value)<=2021) {
                upYear = parseInt(document.getElementById("toYR").value)
            }
        }
		if (parseInt(document.getElementById("frYR").value)!=lowYear){
			alert("Your input for 'From Year' is out of range!");
			document.getElementById("frYR").value=lowYear;
		}	
		if (parseInt(document.getElementById("toYR").value)!=upYear) {
			alert("Your input for 'To Year' is out of range!");
			document.getElementById("toYR").value=upYear;
		}
	...
	var filterMTList = [];
    if (document.getElementById("ftMT1").checked) {
        filterMTList.push("AC");
    }
    if (document.getElementById("ftMT2").checked) {
        filterMTList.push("APC");
    }
    if (document.getElementById("ftMT3").checked) {
        filterMTList.push("PCC");
    }
    if (document.getElementById("ftMT4").checked) {
        filterMTList.push("X");
    }
    if (filterMTList.length==0) {
		filterMTList.push("");
	}
    var filterMT = ["match", ["get", "MT"]];
    filterMT.push(filterMTList);
    filterMT.push(true);
    filterMT.push(false);
    newFilter.push(filterMT);
...
function changeLY() {
    document.getElementById("divLY").style.display = (document.getElementById("chkLY").checked ? "" : "none");
}
...
function exportCSV() {
	...
	var fileContent = "OID,SegmentName,FromStreet,ToStreet,Functional, PCI, Work Completed";
	for (var i=0; i<features.length; i++) {
		fileContent=fileContent +  '\r\n' + features[i].properties.OID + "," + features[i].properties.NM.replaceAll(",", " ") + "," + features[i].properties.FM.replaceAll(",", " ") + "," + features[i].properties.TO.replaceAll(",", " ") + "," + features[i].properties.FN + "," + features[i].properties.PCI + "," + (features[i].properties.YR=="NA"? "": features[i].properties.YR);
	}
